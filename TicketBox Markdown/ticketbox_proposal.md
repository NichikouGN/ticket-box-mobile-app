...
* **Hướng xử lý:** Áp dụng hai lớp kiểm soát:
    1. *Lớp đầu tiên:* Dùng Redis atomic `DECRBY` để giảm bộ đếm số vé còn lại ngay khi nhận request — do Redis xử lý lệnh đơn lẻ theo kiểu single-threaded và atomic, các request đến sau sẽ thấy count đã giảm và bị từ chối ngay, tránh dồn tải vào database.
    2. *Lớp thứ hai:* Với các request vượt qua Redis, tiếp tục dùng `SELECT ... FOR UPDATE` trong PostgreSQL transaction để đảm bảo tính nhất quán tuyệt đối trước khi commit.
* **Ràng buộc:** 
